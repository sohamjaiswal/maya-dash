...
<div class="flex justify-center w-full h-[calc(100vh-8.5rem)]">
<div class="container">
    <main class="px-8 flex flex-col justify-center h-full gap-4 items-center">
      {#if message}
      <p class="text-error-500">
        {message}
      </p>
      {/if}
      <h2 class="h1">Login to Maya</h2>
      ...
      <div class="max-h-80 overflow-y-scroll w-full lg:w-1/2 overflow-x-visible hide-scrollbar">
        <dl class="list-dl mt-5">
          {#each users as user}
          <form action={`?/linkGuilded${redirectTo ? "&redirectTo=" + redirectTo : ''}`} method="POST" class="p-2">
            <label for="guildedId" />
      ...
      <form action={`?/linkGuilded${redirectTo ? "&redirectTo=" + redirectTo : ''}`} method="post" class="flex flex-col gap-4 w-full lg:w-1/2">
        ...
        <button class="btn variant-ghost-surface" type="submit">Link Maya</button>
      </form>
    </main>
  </div>
</div>
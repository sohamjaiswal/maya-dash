<script lang="ts">
	...
	$: preciseClassesActive = (href: string) =>
		$page.url.pathname === href ? '!bg-primary-500' : '';
					...
					{#each servers as server}
						<div class="flex flex-col gap-4">
							<a
								...
								class={`w-full p-2 rounded-lg ${classesActive(`/dashboard/${server.id}`)}`}
							>
							...
							<div class={`flex flex-col gap-2 ml-10 ${tabsActive(`/dashboard/${server.id}`)}`}>
								<a href={`/dashboard/${server.id}`}>
									<div
										class={`w-full p-2 rounded-lg ${preciseClassesActive(
											`/dashboard/${server.id}/general`
										)} `}
									>
										🏠 General
									</div>
